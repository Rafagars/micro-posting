...
def show(slug):
    ...
    post.username = user.username
    page = request.args.get('page', 1, type=int)
    comments = Comment.query.filter_by(post_id=post.id).order_by(Comment.id.desc()).paginate(page=page, per_page=5,
                                                                                             error_out=True)
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            return render_template("post/show.html", post=post, form=form, comments=comments.items, next_url=next_url,
                                   prev_url=prev_url)
